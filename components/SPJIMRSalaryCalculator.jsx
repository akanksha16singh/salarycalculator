...
function calcTax(income) {
  ...
  if (Math.max(0, income - std) <= 12) tax = 0;
  return tax + tax * 0.04;
}
...
export default function SPJIMRSalaryCalculator() {
  ...
  const breakdown = useMemo(() => {
    ...
    return { annual, basic, hra, bonus, esop, other, tax, pf, profTax, totalDeductions, inHand };
  }, [ctcNum, compData]);
  ...
  useEffect(() => {
    ...
    projections.forEach((p, i) => {
      ctx.fillText(`Y${p.year}`, pad.left + (cW / 10) * i, H - pad.bottom + 20);
    });

    // CTC area fill
    const gradient = ctx.createLinearGradient(0, pad.top, 0, pad.top + cH);
    gradient.addColorStop(0, isDark ? "rgba(232,167,88,0.3)" : "rgba(196,124,26,0.25)");
    gradient.addColorStop(1, isDark ? "rgba(232,167,88,0.01)" : "rgba(196,124,26,0.01)");
    ctx.beginPath();
    projections.forEach((p, i) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: SANS, padding: 0, transition: "background 0.3s, color 0.3s" }}>
      <style>{`
      ...
      <div style={{ padding: "32px 24px 0", maxWidth: 880, margin: "0 auto" }}>
        <div className="fade-up" style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 8, justifyContent: "space-between" }}>
        ...
        <div className="fade-up" style={{ animationDelay: "0.1s", marginTop: 24, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
          {[
            ...
            <div key={i} style={{ background: T.benchmarkBg, borderRadius: 10, padding: "14px 12px", textAlign: "center", border: `1px solid ${T.benchmarkBorder}`, cursor: "pointer", transition: "all 0.2s" }}
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "20px 24px 0" }}>
        <div className="fade-up" style={{ animationDelay: "0.2s", display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {[
            { label: "Annual In-Hand", val: fmtLakhs(breakdown.inHand), sub: "After all deductions", color: T.accentGreen },
            { label: "Monthly In-Hand", val: fmtMonthly(breakdown.inHand), sub: "Take-home per month", color: T.accentBlue },
            { label: "Total Tax", val: fmtLakhs(breakdown.tax), sub: `Effective ${((breakdown.tax / breakdown.annual) * 100).toFixed(1)}%`, color: T.accentRed },
          ].map((item, i) => (
            <div key={i} style={{ background: T.bgCard, borderRadius: 12, padding: "18px 16px", border: `1px solid ${item.color}30`, position: "relative", overflow: "hidden", boxShadow: isDark ? "none" : "0 2px 8px rgba(0,0,0,0.06)" }}
              className="card-hover">
        ...
        <div className="fade-up" style={{ animationDelay: "0.25s", display: "flex", gap: 4, background: T.bgTab, borderRadius: 10, padding: 3 }}>
          {[
            ...
            <button key={tab.id} onClick={() => { setActiveTab(tab.id); if (tab.id === "projection") setShowProjection(true); }}
              style={{ flex: 1, padding: "10px 0", borderRadius: 8, border: "none", background: activeTab === tab.id ? T.bgTabActive : "transparent",
                color: activeTab === tab.id ? T.accent : T.textMuted, fontFamily: SANS, fontSize: 13, fontWeight: 500, cursor: "pointer", transition: "all 0.2s",
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "16px 24px 40px" }}>
        ...
        {activeTab === "breakdown" && (
          <div className="fade-up">
            ...
            {/* Breakdown table */}
            <div style={{ background: T.bgCard, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden", boxShadow: isDark ? "none" : "0 2px 8px rgba(0,0,0,0.05)" }}>
              {[
                { label: "Basic Salary", annual: breakdown.basic, color: T.accent },
                { label: "HRA", annual: breakdown.hra, color: T.accentGreen },
                { label: "Performance Bonus", annual: breakdown.bonus, color: T.accentBlue },
                { label: "ESOPs / RSUs", annual: breakdown.esop, color: T.accentPurple },
                { label: "Other Allowances", annual: breakdown.other, color: T.accentRed },
              ].map((row, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", padding: "14px 20px", borderBottom: `1px solid ${T.rowBorder}`, alignItems: "center" }}>
              ...
              <div style={{ height: 1, background: `${T.accent}33`, margin: "0 20px" }} />
              ...
              <div style={{ padding: "8px 20px 4px" }}>
                <span style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: T.textFaint }}>Deductions</span>
              </div>
              {[
                ...
                <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", padding: "10px 20px", alignItems: "center" }}>
                  <span style={{ fontSize: 13, color: T.deductionText }}>− {row.label}</span>
        ...
        {activeTab === "domain" && (
          <div className="fade-up">
            <div style={{ background: T.bgCard, borderRadius: 12, border: `1px solid ${T.border}`, padding: 20, marginBottom: 16, boxShadow: isDark ? "none" : "0 2px 8px rgba(0,0,0,0.05)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontFamily: FONT, fontSize: 28, fontStyle: "italic", color: ctcNum >= domainData.top20 ? T.accentGreen : ctcNum >= domainData.median ? T.accentBlue : T.accentRed }}>
                ...
                {[
                  ...
                  <div key={i}>
                    ...
                    <div style={{ height: 6, background: T.barTrack, borderRadius: 3, overflow: "hidden", position: "relative" }}>
                      ...
                      <div style={{ position: "absolute", top: -3, left: `${Math.min((ctcNum / domainData.top20) * 100, 100)}%`, width: 2, height: 12, background: T.text, borderRadius: 1, transform: "translateX(-50%)", opacity: 0.7 }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ background: T.bgCard, borderRadius: 12, border: `1px solid ${T.border}`, padding: 20, boxShadow: isDark ? "none" : "0 2px 8px rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {domainData.companies.map((c, i) => (
                  <span key={i} style={{ padding: "6px 14px", borderRadius: 20, background: T.tagBg, border: `1px solid ${T.tagBorder}`, fontSize: 12, color: T.tagText }}>
                    {c}
                  </span>
                ))}
              </div>
            ...
            <div style={{ marginTop: 16, background: T.bgCard, borderRadius: 12, border: `1px solid ${T.border}`, padding: 20, boxShadow: isDark ? "none" : "0 2px 8px rgba(0,0,0,0.05)" }}>
        ...
        {activeTab === "projection" && (
          <div className="fade-up">
            <div style={{ background: T.bgCard, borderRadius: 12, border: `1px solid ${T.border}`, padding: "20px 16px 16px", marginBottom: 16, boxShadow: isDark ? "none" : "0 2px 8px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ background: T.bgCard, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden", boxShadow: isDark ? "none" : "0 2px 8px rgba(0,0,0,0.05)" }}>
              ...
              {projections.map((p, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "0.7fr 1fr 1fr 1fr 1fr", padding: "10px 16px", borderBottom: `1px solid ${T.projRowBorder}`, background: i === 0 ? T.projRowHighlight : "transparent" }}>
            ...
            <div style={{ marginTop: 12, padding: "12px 16px", background: T.noteBox, borderRadius: 10, border: `1px solid ${T.noteBorder}` }}>
              <p style={{ fontSize: 11, color: T.textMuted, lineHeight: 1.5 }}>
                <span style={{ color: T.accent }}>Note:</span> Projections assume ~{domainData.growth}% annual growth for years 1–3, tapering to ~{(domainData.growth * 0.85).toFixed(0)}% for years 4–6, and ~{(domainData.growth * 0.7).toFixed(0)}% for years 7–10. Tax computed under the New Tax Regime FY 2025-26.
              </p>
            </div>
          </div>
        )}
